...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Exibir vídeo do YouTube automaticamente
    loadYouTubeVideo('1YfuzqeZp3Y');
    ...
    if (menuBtn && sideDrawer && drawerBackdrop && closeDrawer) {
...
function initializeHexagons() {
    ...
    const hexSize = 70; // menor para mais espaçamento
    const hexWidth = hexSize;
    const hexHeight = hexSize * 0.866; // Altura do hexágono
    const spacing = 28; // espaçamento entre hexágonos
    
    // Calcular número de hexágonos baseado no tamanho da tela
    const cols = Math.floor((containerWidth + spacing) / (hexWidth + spacing * 0.5));
    const rows = Math.floor((containerHeight + spacing) / (hexHeight + spacing * 0.5));
    const totalHexagons = Math.min(cols * rows, 150); // mais hexágonos para preencher
    
    // Criar hexágonos
    for (let i = 0; i < totalHexagons; i++) {
        const hexagon = document.createElement('div');
        hexagon.className = 'hexagon';
        // Forçar visual preto brilhante
        hexagon.style.background = 'linear-gradient(145deg, #232323 60%, #181818 100%)';
        hexagon.style.border = '2px solid rgba(212, 175, 55, 0.08)';
        hexagon.style.boxShadow = '0 2px 12px 0 rgba(255,255,255,0.04), 0 0 0 0 rgba(212,175,55,0)';
        
        // Posicionamento em grade hexagonal com espaçamento
        const col = i % cols;
        const row = Math.floor(i / cols);
        const x = col * (hexWidth + spacing * 0.5);
        const y = row * (hexHeight + spacing) + (col % 2) * ((hexHeight + spacing) / 2);
